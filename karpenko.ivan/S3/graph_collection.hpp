...
private:
  GraphMap graphs_;
public:
  bool hasGraph(const std::string& name) const { return graphs_.has(name); }
  Graph& getGraph(const std::string& name)
  {
    auto it = graphs_.find(name);
    if (it == graphs_.end()) throw std::runtime_error("Graph not found");
    return it->second;
  }
  ...
  {
    if (graphs_.has(name)) throw std::runtime_error("Graph already exists");
    graphs_.add(name, g);
  }
  GraphMap& getMap() { return graphs_; }
  const GraphMap& getMap() const { return graphs_; }
};
...
GraphCollection readGraphs(const std::string& filename)
{
  ...
  while (std::getline(file, line))
  {
    ...
    for (std::size_t i = 0; i < edge_count; ++i)
    {
      ...
      std::string from, to;
      Weight w;
      if (!(edge_stream >> from >> to >> w)) continue;
