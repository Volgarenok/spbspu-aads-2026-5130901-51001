...
void handleGraphs(std::istream&, std::ostream& out, GraphCollection& gc)
{
  ...
  if (n == 0)
    ...
    for (std::size_t i = 0; i < n; ++i)
      out << arr[i] << '\n';
  for (std::size_t i = 0; i < n; ++i) arr[i].~basic_string();
  ::operator delete(arr);
}
...
void handleVertexes(std::istream& in, std::ostream& out, GraphCollection& gc)
{
  std::string gname;
  in >> gname;
  if (!in || !gc.hasGraph(gname)) { out << "<INVALID COMMAND>\n"; return; }
...
void handleCut(std::istream& in, std::ostream& out, GraphCollection& gc)
{
  ...
  Weight w{};
  in >> gname >> a >> b >> w;
  if (!in || !gc.hasGraph(gname) || !gc.getGraph(gname).vertices_.has(a) || !gc.getGraph(gname).vertices_.has(b))
  { out << "<INVALID COMMAND>\n"; return; }
  Graph graph = gc.getGraph(gname);
  if (!hasWeightInMap(graph.out_edges_, a, b, w) || !hasWeightInMap(graph.in_edges_, b, a, w))
  { out << "<INVALID COMMAND>\n"; return; }
...
void handleExtract(std::istream& in, std::ostream& out, GraphCollection& gc)
{
  ...
  {
    std::string v;
    if (!(in >> v) || !source.vertices_.has(v)) { out << "<INVALID COMMAND>\n"; return; }
  ...
  for (auto sit = selected.begin(); sit != selected.end(); ++sit)
  {
    ...
    if (out_it == source.out_edges_.end()) continue;
    const InnerEdgeMap& inner = out_it->second;
...
void CommandSystem::execute(const std::string& line, std::ostream& out, GraphCollection& gc)
{
  ...
  if (it == handlers_.end()) { out << "<INVALID COMMAND>\n"; }
  else
  {
    try { it->second(iss, out, gc); }
