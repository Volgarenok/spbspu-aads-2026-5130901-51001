...
namespace loseva {
  ...
  namespace detail {
    ...
    template<typename T>
    typename List<T>::Node* create_node(T&& value, typename List<T>::Node* next = nullptr, typename List<T>::Node* prev = nullptr) {
      return new typename List<T>::Node(std::move(value), next, prev);
    }
    template<typename T, typename... Args>
    typename List<T>::Node* create_node(Args&&... args, typename List<T>::Node* next = nullptr, typename List<T>::Node* prev = nullptr) {
      return new typename List<T>::Node(T(std::forward<Args>(args)...), next, prev);
    }
  }
  ...
  List<T>::List(InputIt first, InputIt last) : head(nullptr), tail(nullptr), size(0) {
