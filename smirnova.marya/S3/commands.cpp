...
{
  Vector<std::string> names; for (auto it = graphs.begin(); it != graphs.end(); ++it) names.pushBack(it->key);
  sortStrings(names); if (names.size() == 0) { out << "\n"; return; }
...
void vertexes(std::istream&, std::ostream& out, GraphTable&, VertTable& graphVertices, std::string graphName)
{
  ...
  Vector<std::string> verts = graphVertices.get(graphName); sortStrings(verts);
  bool printed = false; std::string prev; bool first = true;
  for (auto it = verts.begin(); it != verts.end(); ++it) if (first || *it != prev) { out << *it << "\n"; prev = *it; first = false; printed = true; }
  if (!printed) out << "\n";
}
...
void create(std::istream& in, std::ostream& out, GraphTable& graphs, VertTable& graphVertices, std::string graphName)
{
  ...
  size_t k = 0; Vector<std::string> verts; if (!(in >> k)) { in.clear(); } else { for (size_t i = 0; i < k; ++i) { std::string v; in >> v; verts.pushBack(v); } }
  Graph g; graphs.add(graphName, g); graphVertices.add(graphName, verts);
}

void bind(std::istream& in, std::ostream& out, GraphTable& graphs, VertTable& graphVertices, std::string graphName)
{
  std::string a, b; int w; in >> a >> b >> w;
  if (!graphs.has(graphName)) { out << "<INVALID COMMAND>\n"; return; }
  if (!graphVertices.has(graphName)) { Vector<std::string> verts; graphVertices.add(graphName, verts); }
  Vector<std::string>& verts = graphVertices.get(graphName); bool hasA = false, hasB = false;
  for (auto it = verts.begin(); it != verts.end(); ++it) { if (*it == a) hasA = true; if (*it == b) hasB = true; }
  if (!hasA) verts.pushBack(a); if (!hasB) verts.pushBack(b); graphs.get(graphName).addEdge(a, b, w);
}
...
void cut(std::istream& in, std::ostream& out, GraphTable& graphs, VertTable& graphVertices, std::string graphName)
{
  ...
  Graph& g = graphs.get(graphName); if (!g.adj.has(a)) { out << "<INVALID COMMAND>\n"; return; }
  Vector<Graph::Edge>& edges = g.adj.get(a); bool foundEdge = false;
  for (auto eit = edges.begin(); eit != edges.end(); ++eit)
  {
    if (eit->to != b) continue;
    Vector<int>& weights = eit->weights; Vector<int> newWeights; bool removed = false, skipped = false;
    for (auto wIt = weights.begin(); wIt != weights.end(); ++wIt)
    {
      if (!skipped && *wIt == w) { skipped = true; removed = true; continue; }
      newWeights.pushBack(*wIt);
    }
    if (!removed) { out << "<INVALID COMMAND>\n"; return; }
    eit->weights = newWeights; foundEdge = true; break;
  }
...
void outbound(std::istream& in, std::ostream& out, GraphTable& graphs, VertTable& graphVertices, std::string graphName)
{
  std::string v; in >> v;
  if (!graphs.has(graphName) || !graphVertices.has(graphName)) { out << "<INVALID COMMAND>\n"; return; }
  Vector<std::string>& verts = graphVertices.get(graphName); bool hasV = false;
  for (auto it = verts.begin(); it != verts.end(); ++it) { if (*it == v) { hasV = true; break; } }
  if (!hasV) { out << "<INVALID COMMAND>\n"; return; }
  Graph& g = graphs.get(graphName); if (!g.adj.has(v)) { out << "\n"; return; }
  struct OutRes { std::string to; Vector<int> weights; }; Vector<OutRes> results; Vector<Graph::Edge>& edges = g.adj.get(v);
  for (auto eit = edges.begin(); eit != edges.end(); ++eit)
  {
    bool found = false;
  ...
  for (size_t i = 0; i < results.size(); ++i)
    for (size_t j = i + 1; j < results.size(); ++j)
      if (results[j].to < results[i].to) { OutRes tmp = results[i]; results[i] = results[j]; results[j] = tmp; }
...
void inbound(std::istream& in, std::ostream& out, GraphTable& graphs, VertTable& graphVertices, std::string graphName)
{
  ...
  for (auto it = verts.begin(); it != verts.end(); ++it) { if (*it == v) { hasV = true; break; } }
  if (!hasV) { out << "<INVALID COMMAND>\n"; return; }
  Graph& g = graphs.get(graphName); struct InRes { std::string from; Vector<int> weights; }; Vector<InRes> results;
  for (auto it = g.adj.begin(); it != g.adj.end(); ++it)
  {
    const std::string& from = it->key; Vector<Graph::Edge>& edges = it->value;
    for (auto eit = edges.begin(); eit != edges.end(); ++eit)
    {
...
void merge(std::istream& in, std::ostream& out, GraphTable& graphs, VertTable& graphVertices, std::string graphName)
{
  ...
  if (graphs.has(graphName) || !graphs.has(g1) || !graphs.has(g2) || !graphVertices.has(g1) || !graphVertices.has(g2)) { out << "<INVALID COMMAND>\n"; return; }
  Graph result; Graph& a = graphs.get(g1); Graph& b = graphs.get(g2); appendEdges(result, a); appendEdges(result, b); graphs.add(graphName, result);
  Vector<std::string> verts; Vector<std::string>& va = graphVertices.get(g1); Vector<std::string>& vb = graphVertices.get(g2);
...
void extract(std::istream& in, std::ostream& out, GraphTable& graphs, VertTable& graphVertices, std::string graphName)
{
  ...
  Graph& src = graphs.get(oldG); Vector<std::string>& srcVerts = graphVertices.get(oldG); Vector<std::string> chosen;
  for (size_t i = 0; i < k; ++i)
  {
    std::string v; in >> v; bool found = false;
    for (auto it = srcVerts.begin(); it != srcVerts.end(); ++it) { if (*it == v) { found = true; break; } }
    if (!found) { out << "<INVALID COMMAND>\n"; return; }
    appendUnique(chosen, v);
  }
  Graph res; Vector<std::string> resVerts;
  for (auto it = chosen.begin(); it != chosen.end(); ++it) { resVerts.pushBack(*it); res.addVertex(*it); }
  for (auto it = chosen.begin(); it != chosen.end(); ++it)
  {
    const std::string& from = *it; if (!src.adj.has(from)) continue; Vector<Graph::Edge>& edges = src.adj.get(from);
    for (auto eIt = edges.begin(); eIt != edges.end(); ++eIt)
    {
  ...
  graphs.add(graphName, res); graphVertices.add(graphName, resVerts);
}
